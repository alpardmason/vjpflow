...
- :meth:`Primitive.forward` -- *what to compute*, delegated to the backend. This
  runs during :func:`autograd.graph.evaluate`.
...
(``__add__`` etc.) are defined in :mod:`autograd.tensor` and dispatch here.
...
from autograd.backends import Native
...
# why ``autograd/__init__.py`` imports ``primitives`` eagerly.
