...
def linear(x: Tensor, weight: Tensor, bias: Tensor | None = None) -> Tensor:
    ...
    produced automatically by :class:`~autograd.primitives.MatMul` and the
    broadcast-aware :class:`~autograd.primitives.Add`.
    """
    y = P.matmul(x, weight)
...
def embedding(table: Tensor, indices: np.ndarray) -> Tensor:
    ...
    is exactly :class:`~autograd.primitives.Gather`'s vjp.
    """
