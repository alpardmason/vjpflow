...
- **Whole-graph view before execution.** Because the graph exists as data
  ...
  better kernels, allocate buffers once. That is what :mod:`vjpflow.jit` does.
- **One autodiff mechanism.** ``grad`` builds a *backward graph* out of the same
  ...
  transformation (see :mod:`vjpflow.autodiff`).

The operator overloads (``__add__`` and friends) are attached at import time by
:mod:`vjpflow.primitives`, to avoid a circular import (primitives need
...
from vjpflow.backends import Backend, Native, default_backend

if TYPE_CHECKING:
    from vjpflow.primitives import Primitive
...
class Tensor:
    ...
    op:
        The :class:`~vjpflow.primitives.Primitive` that produces this node, or
    ...
    def eval(self) -> Tensor:
        ...
        from vjpflow.graph import evaluate
    ...
    # -- operators -------------------------------------------------------
    # These dispatch into the functional ops in ``vjpflow.primitives``. We
    ...
    def T(self) -> Tensor:  # noqa: N802  (conventional transpose attribute, as in numpy/torch)
        ...
        from vjpflow.primitives import _swap_last_two
