"""VJPFlow — a tiny, educational lazy autograd engine.
...
Public API
----------
- :class:`~vjpflow.tensor.Tensor` -- a node in the lazy graph.
- :func:`~vjpflow.tensor.tensor` -- build a leaf tensor from data.
- :func:`~vjpflow.autodiff.grad` / :func:`~vjpflow.autodiff.value_and_grad`
  -- functional differentiation transforms.
- The functional ops (``add``, ``matmul``, ``softmax`` ...) live in
  :mod:`vjpflow.primitives` and :mod:`vjpflow.nn.functional`.
"""

from __future__ import annotations

from vjpflow import primitives as primitives  # noqa: F401  (registers Tensor operators)
from vjpflow.autodiff import grad, value_and_grad
from vjpflow.backends import (
    Backend,
    default_backend,
    get_backend,
    set_default_backend,
